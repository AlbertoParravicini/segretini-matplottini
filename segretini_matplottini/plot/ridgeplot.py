...
def _setup_plot(
    ...
    # Plotting setup;
    if reset_plot_style:
        _reset_plot_style(label_pad=7, border_width=0.8)
...
def ridgeplot(
    ...
    _style_fine_tuning(
        g=g,
        xlimits=xlimits,
        x_axis_line_width=1.4,
    )
    _add_individual_plot_labels(
        g=g,
        plot_label_x_position=0.005,
    ...
    for ax_i in axes:
        for ax_j in ax_i:
            ...
            ax_j.tick_params(axis="x", which="major", labelsize=font_size - 1)
            for tic in ax_j.xaxis.get_major_ticks():
...
def ridgeplot_compact(
    ...
    # Add custom legend;
    _add_legend(palette=palette, legend_labels=legend_labels, g=g, font_size=font_size + 1)
