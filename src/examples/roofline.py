...
def roofline(performance: list, operational_intensity: list, 
             peak_performance: list, peak_bandwidth: list,
             ax=None,
             setup: bool=True,
             xmin: float=None, xmax: float=None,
             ymin: float=None, ymax: float=None,
             palette: list=None, markers: list=None,
             base_font_size: int=6,
             xlabel: str="Operational Intensity [{}/s]",
             ylabel: str="Performance [{}/B]",
    ...
    for i in range(num_rooflines):
        ...
        _xmax = np.max([ax.get_xlim()[1], line_cross * 10]) if not xmax else xmax # Show 10 times more than the minimum operational intensity that causes a compute-bound algorithm;
        _ymin = peak_performance[i] / 10000 if not ymin else ymin  # By default, show up to 1/10000 of peak performance;
        ...
        # Plot lines;    
        plt.plot([operational_intensity[i], operational_intensity[i]], [0, line_cross_op_intensity],
                  color="#757575", linewidth=0.5, linestyle="--", zorder=0)
        plt.plot([0, _xmax], [peak_performance[i], peak_performance[i]],
                  color="#757575", linewidth=0.5, linestyle="--", zorder=0)
        plt.plot([line_cross, _xmax], [peak_performance[i], peak_performance[i]],
                  color=color, linewidth=1, zorder=1)
        plt.plot([0, line_cross], [0, peak_performance[i]], color=color, linewidth=1, zorder=1)
        plt.scatter(operational_intensity[i], performance[i], color=color, edgecolors="#2f2f2f", 
                    marker=marker, s=SCATTER_SIZE, zorder=2, linewidth=0.3)
        ...
        if add_bandwidth_label:
            ...
            ax.annotate(label, fontsize=base_font_size * 0.75, xy=(x_loc, peak_bandwidth[i] * x_loc * 1.1), 
                        ha="left", rotation_mode='anchor', rotation=trans_angle, color="#2f2f2f")
        ...
        if add_operational_intensity_label:
            ...
            ax.annotate(label, fontsize=base_font_size, xy=(operational_intensity[i] * 1.1, ax.get_ylim()[0] * 2), ha="left")
    ...
    if add_legend:
        ...
        custom_lines = [Line2D([], [], color="white", marker=markers[j] if markers else MARKERS[0], markeredgewidth=0.5, 
                       markersize=5, label=legend_labels[j], markerfacecolor=palette[j] if palette else PALETTE[0], markeredgecolor="#2f2f2f") for j in range(len(legend_labels))]
        leg = ax.legend(custom_lines, legend_labels, bbox_to_anchor=(0.95, 0), fontsize=base_font_size, ncol=1, loc="lower center", handletextpad=0.3, columnspacing=0.4)
        leg.set_title(None)
...
if __name__ == "__main__":
    ...
    ax = roofline(performance, operational_intensity, peak_performance, peak_bandwidth,
                  palette=PALETTE, markers=MARKERS, 
                  performance_unit="FLOPS",
                  xmin=0.01, xmax=20,
                  add_legend=True, legend_labels=[f"{c} Cores" for c in num_cores])
    ...
    ax = roofline(performance, operational_intensity, peak_performance, peak_bandwidth,
                  ...
                  ax=ax,
                  performance_unit="FLOPS",
                  xmin=0.01, xmax=20, ylabel="",
                  add_legend=True, legend_labels=["CPU", "GPU", "FPGA"],
                  add_bandwidth_label=False, add_operational_intensity_label=False)
